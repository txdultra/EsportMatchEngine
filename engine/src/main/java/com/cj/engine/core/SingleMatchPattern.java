...
import com.cj.engine.core.cfg.SingleMPCfg;
...
public class SingleMatchPattern extends AbstractMatchPattern {
    ...
    public SingleMatchPattern(MPCfg cfg) {
        super(cfg);
        if(!(cfg instanceof SingleMPCfg)) {
            throw new IllegalArgumentException("单淘赛制配置对象类型必须是SingleMPCfg");
        }
        if(cfg.getGroupPlayerCount() != 2) {
            throw new IllegalArgumentException("单淘赛制只支持小组人数为2");
        }
        SingleMPCfg sCfg = (SingleMPCfg)cfg;
        this.hasChampion = sCfg.isHasChampion();
        this.hasThrider = sCfg.isHasThrider();
    ...
    protected int calculateMaxRounds(int counts) {
        ...
        while(maxRGs > 1) {
            //到达此轮已完成本赛制晋级人员数量
            if(!this.hasChampion) {
                if(this.cfg.getPromotionCounts() >= maxRGs * this.cfg.getGroupPlayerCount()) {
                    break;
                }
            }
            rounds++;
    ...
    private void setNodesRel() {
        for(int i = 1; i <= this.rounds.size(); i++) {
            ...
            if(this.hasThrid && (i - 1) == this.rounds.size()) {
    ...
    @Override
    protected MResult newSchedule(int players) {
        if(players <= 0)
